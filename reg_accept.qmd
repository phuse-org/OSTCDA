...
Perhaps it is down to fear of having a dossier rejected by a regulatory agency - "Refusal to file" - due to application deficiencies. What if an agency found discrepancies between an analysis performed by the sponsor with an open-source software and their own analysis (whether using the same open-source software or some other proprietary software)? Would this warrant a "Refusal to file"? Since "Refusal to file" could have a significant impact on a company's reputation, confidence and ultimately their stock price, it is understandable that companies are cagey. Also, who would want to be first to try out this untrodden path? It is easier to follow in the footsteps of somebody else than to forge a path yourself.
